...
Select file in "${name}" with "${image_name}"
  ...
  Upload File By Selector   ${element}                        test/upload/${image_name}

Click radio "${name}" in line "${text}"
  ${element}=               Get Element Form Item By Name     ${name}                       //*[contains(@class, "ant-radio-wrapper-in-form-item")]/span[contains(text(), "${text}")]
  Click                     ${element}
  ...
  IF    ${cnt} > 0
    ...
    ELSE
      ...
      IF    ${cnt2} > 0
        Get Text            ${element}                         equal                        ${value}
      ELSE
        ${element}=         Get Element Form Item By Name      ${name}                       //*[contains(@class, "ant-radio-wrapper-in-form-item") and contains(@class,"ant-radio-wrapper-checked")]/span[contains(text(), "${value}")]
        Wait Until Element Is Existent                         ${element}
      END
    END
  ELSE
